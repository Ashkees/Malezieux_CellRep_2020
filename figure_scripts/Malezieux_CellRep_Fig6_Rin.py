# -*- coding: utf-8 -*-
"""
Created on Mon Dec 16 15:11:57 2019
...
# Figure 6
# Description: mechanisms of theta hyperpolarization: input resistance
...
# %% try to correlate change in Rin with theta hyp cells

theta_hyp = np.array([np.nan, np.nan, np.nan, np.nan, np.nan, np.nan,
                      0, 0, 1, 0, 1, 1, 0])

all_cells = np.full(len(data), np.nan)
for i in np.arange(len(data)):
    cell_Rin_nost = np.nanmean(data[i]['Rin'][data[i]['nost_Rin']])
    cell_Rin_state = np.nanmean(data[i]['Rin'][data[i][state[l]+'_Rin']])
    cell_Rin_state = (cell_Rin_state-cell_Rin_nost)/cell_Rin_nost
    all_cells[i] = cell_Rin_state*100
    
g0 = all_cells[theta_hyp == 0]
g1 = all_cells[theta_hyp == 1]
boot_t(g0, g1, 1000)



LIA_dep = np.array([np.nan, np.nan, np.nan, np.nan, np.nan, np.nan,
                    0, 0, 0, 0, 1, 0, 0])
# can't do stats for 1 LIA dep cell
