# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 17:37:51 2019
...
# Figure S7
# Description: mechanisms of LIA depolarization: variance
...
m = 1
states = ['unlabeled', 'LIA',]
if measure == 'dVm_dvar':
    bins = np.arange(-5, 5.25, 0.01)
if measure == 'dVm':
    bins = np.arange(-5, 0.01, 0.01)
if measure == 'dvar':
    bins = np.arange(0, 10, 0.01)
if measure == 'Vm1_var1':
    bins = np.arange(-0.5, 0, 0.001)
if measure == 'var1':
    bins = np.arange(0, 20, 0.01)
if measure == 'Vm1':
    bins = np.arange(-80, -20, 0.01)
fig, ax = plt.subplots(1, 1, sharex=True, figsize=[1.8, 1.9])
plt.hist([events[m][measure][events[m][states[1]+'_bool']],
          events[m][measure][events[m][states[0]+'_bool']]], bins=bins,
          color=[c_bwn, c_mgry], density=True, 
