# -*- coding: utf-8 -*-
"""
Created on Sun Dec 15 16:19:19 2019
...
fig_folder = r'C:\Users\akees\Documents\Ashley\Figures\2020-05_Paper_MIND1\Fig5'


# %% make figures - dVm histograms by event
...
# over all events, show that there are more/less sig hyp/dep runs than expected
...
ax.hist([hyp_sig, hyp_no], bins=bins, color=[c_hyp, c_no], stacked=True,
        label = [' ',' '])
ax.hist([dep_sig, dep_no], bins=bins, color=[c_dep, c_no], stacked=True,
        label = [' '])
ax.set_ylim([0, 30])
...
# histogram of delta Vm values with color for sig changes and white for not
ax.hist([cell_hyp_sig, cell_hyp_no], bins=bins, color=[c_hyp, c_no], stacked=True,
        label = [' ', ' '])
ax.hist([cell_dep_sig, cell_dep_no], bins=bins, color=[c_dep, c_no], stacked=True,
        label = [' '])
ax.set_ylim([0, 12])
ax.set_xlim([-8, 8])
...
for i in np.arange(len(data)):
    if data[i][state+'_start'].size > 0:
        H_c, bins = np.histogram(np.concatenate(data[i][state+'_spike']), bins)
        H_c = H_c/(len(data[i][state+'_sp'])*bin_width)
        x = H_c[0:8]
        y = H_c[12:20]
        fr_bef[i] = np.mean(x)
        fr_aft[i] = np.mean(y)
...
all_dVm = np.array([d[state+'_mean_dVm'] for d in data])[unique_cells]
all_cell_p = np.array([d[state+'_cell_p'] for d in data])[unique_cells]
all_dVm = all_dVm[~np.isnan(fr_diff)]
...
ax.set_ylabel(r'mean $\Delta$ firing rate (Hz)', labelpad=6)        
...
# plot the raster
fig, axs = plt.subplots(4, 1, sharex=True, figsize=(1.75, 3.55),
...
axs[1].bar(bins[:-1], H, width=bin_width, color=c_dep, align='edge')
axs[1].axis('tight')
axs[1].set_yticks([5, 10])
axs[1].set_xticks([-2, 0])
...
H, bins = np.histogram(np.concatenate(raster), bins)
H = H/(len(raster)*bin_width)
axs[3].bar(bins[:-1], H, width=bin_width, color=c_hyp, align='edge')
axs[3].axis('tight')
axs[3].set_yticks([5, 10])
axs[3].set_xticks([-2, 0])
axs[3].set_ylabel('FR (Hz)')
axs[3].set_xlabel('time relative to LIA onset (s)')
plt.tight_layout()
